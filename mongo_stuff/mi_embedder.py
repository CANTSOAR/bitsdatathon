...
def convert_date_to_timestamp(date_str):
    """Converts date string to UNIX timestamp (integer)."""
    try:
        return int(datetime.strptime(date_str, "%b. %d, %Y, %I:%M %p").timestamp())  # Example: "Mar. 1, 2025, 05:14 PM"
    except ValueError:
        try:
            return int(datetime.strptime(date_str, "%Y-%m-%dT%H:%M:%S.%fZ").timestamp())  # ISO format fallback
        except ValueError:
            print(f"⚠️ Date conversion failed for: {date_str}")
            return None
